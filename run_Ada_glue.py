...
def train(args, train_dataset, model, tokenizer,  kd_loss="kd"):
    ...
    tr_fea_loss, logging_fea_loss = 0.0, 0.0
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    tb_writer.add_scalar('fea_loss', (tr_fea_loss - logging_fea_loss)/args.logging_steps, global_step)
                    tb_writer.add_scalar('multi_loss', (tr_multi_loss - logging_multi_loss)/args.logging_steps, global_step)
                    logging_loss = tr_loss
                    logging_last_loss = tr_fea_loss
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                if eval_threshold:
                    exit_layer_counter[outputs[-2]] += 1
                    attention_sentence_lengths = outputs[-1]
                    # print('attention_sentence_lengths ',attention_sentence_lengths)
                    ffn_sentence_lengths = outputs[-1]
                    flops =compute_macs(attention_sentence_lengths,ffn_sentence_lengths,768, args.max_seq_length)
                    actual_flops =actual_flops+flops
        ...
        # print("FLOPs per sample in average: {}".format(total_flops / float(len(eval_dataloader))))
        ...
        if eval_threshold:
            ...
            full_flops = len(eval_dataloader) * _layer_mac(args.max_seq_length, args.max_seq_length, 768) * model.num_layers
            print("speed",  full_flops/actual_flops )
